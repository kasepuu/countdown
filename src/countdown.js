...
class CountdownManager {
    ...
    static loadCountdowns() {
        ...
        savedCountdowns.forEach(({ title, date, id, isGlobal }) => {
            const countdown = new Countdown(title, date, id);
            if (isGlobal) {
                countdown.id = `global-${countdown.id}`;
            }
    ...
    static handleAddCountdown() {
        ...
        if (!title) {
            this.showError(titleInput, 'Please enter a title for your countdown');
            return;
        }
        ...
        if (!time) {
            this.showError(hoursSelect, 'Please select a time');
            return;
        }
    ...
    static saveCountdowns() {
        const countdownsData = this.countdowns.map(c => ({
            ...
            id: c.id,
            isGlobal: c.id.startsWith('global-')
        }));
